...
string __stdcall _IntToHex(int i)
{
	...
	//std::string hexstring;   //= stringFormat("%0X", hex);
...
string __stdcall logStr2Hex(LPSTR  pStrData, int pStrDataLength)
	{
		...
		for (int i = 0; i < pStrDataLength; i++, pStrData++)
		{
			ic = *pStrData & 0x00FF;;
			strHexData = strHexData + "x" + _IntToHex(ic);
		}
...
string __stdcall _WStringToString(const std::wstring& s)
{
	std::string str;
	CW2A cw2a(s.c_str());
	str = cw2a;
...
wstring __stdcall _StringToWString(const std::string& s)
{
...
string __stdcall _ExtractFile(string str)
{
	...
	wStr = PathFindFileName(_StringToWString(str).c_str());     //檔名不包含路徑
		...
		do
		{
			if (FileInformation.cFileName[0] != '.')
			{
				...
				else
				{
					...
					_Strprintf(strCrtDt, "%04d%02d%02d", utc.wYear, utc.wMonth, utc.wDay);
					...
					if (!strDt.empty() && strDt.length() == 8)
					{
						if (strCrtDt.compare(strDt) >= 0)
						{
							...
							_Strprintf(slog, "push_back:File:%s, -> %d-%d-%d Week=%d", str.c_str(), utc.wYear, utc.wMonth, utc.wDay, utc.wDayOfWeek);
							iCnt++;
						}
					...
					else
					{
						...
						lsFileList->push_back(str);
						_Strprintf(slog, "push_back:File:%s", str.c_str());
						iCnt++;
...
/// <returns>int</returns>
int __stdcall _DeleteDirectory(wstring refcstrRootDirectory, bool bDeleteSubdirectories)
{
	...
	refcstrRootDirectory = _StringToWString(str);
...
void __stdcall _DelTree(string dirName_in)
{
	...
	_DeleteDirectory(_StringToWString(dirName_in));  //用更名備份來代替刪目錄
	return;
...
void __stdcall _wMtnDirFile_Ex(wstring wdirName_in, int MtnDay, int FileCnt)
{
	...
	stmp = _WStringToString(wdirName_in);
	...
	for (list<string>::iterator ls = tsl->begin(); ls != tsl->end(); ls++)
	{
		...
		DeleteFile(_StringToWString(str).c_str()); // sDir + mtn_file);
...
void __stdcall _MtnDirFile_Ex(string dirName_in, int MtnDay, int FileCnt)
{
	...
	tsl->clear();
	_GetFileList(tsl, _StringToWString(dirName_in), L"*.*", MtnDay, FileCnt);
	...
	for (list<string>::iterator ls = tsl->begin(); ls != tsl->end(); ls++)
	{
		...
		DeleteFile(_StringToWString(mtn_file).c_str());
...
wstring __stdcall _ReadOneLine(FILE* File, wstring Line)
{
	...
	wchar_t *LineOfChars;
	LineOfChars = (wchar_t*)malloc(4096);
...
vector <wstring> __stdcall _WStringSegment_VT(string str, string delimiter, int seg, int idelSt)
{
	...
	wstring ws = _StringToWString(str);
	wstring wdelimiter = _StringToWString(delimiter);
...
{
	DWORD wRtn, dwStartingPos;
	//TCHAR   log_name[128];
	BOOL fAppend, fVisual;


	if (strlen < 1)
		return;

	//sprintf_s(log_name, "%s", TmpFileName.c_str());
	fAppend = true;
	fVisual = false;
	HANDLE hFile;
...
openagain:
	...
	hFile = CreateFile(_StringToWString(TmpFileName).c_str(),
	...
	if (fAppend)
	{
		if (hFile == INVALID_HANDLE_VALUE)  //001
		{
			//
			...
			if (hFile == INVALID_HANDLE_VALUE)  //002
			{
				if (fVisual)
				{
					goto openagain;
			...
			}  //end of if (hFile == INVALID_HANDLE_VALUE)  002
		}   //end of if (hFile == INVALID_HANDLE_VALUE)  001
  ...
  // Assert( hFile != INVALID_HANDLE_VALUE );
	...
	DWORD   cb;
	cb = strlen;
	WriteFile(hFile, pstr, cb, &cb, NULL);
	CloseHandle(hFile);
	return;
...
string  __stdcall _currentDateTime(int iDttmTypw, int OffsetDay)
{
	...
	case 5:
		...
		break;
	case 6:
		strftime(buf, sizeof(buf), "%H%M%S", &tstruct);  //%H 24小時制的小時 %M 十時製表示的分鐘數 %S 十進制的秒數
		break;
	default:
		strftime(buf, sizeof(buf), "%Y-%m-%d %X", &tstruct);
